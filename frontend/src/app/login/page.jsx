...
function LoginContent() {
  ...
  const handleConnect = async () => {
    setLoading(true);
    try {
      const res = await fetchApi('/auth/instagram/url?user_id=anonymous');
    ...
    <main className="mx-auto flex min-h-[70vh] max-w-md flex-col items-center justify-center px-4">
      <div className="w-full rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-8">
        ...
        <button
          ...
          disabled={loading}
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-[var(--accent)] py-3 font-medium text-white transition hover:bg-[var(--accent-hover)] disabled:opacity-50"
        >
